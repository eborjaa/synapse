...
`oracle` (the read-only Q&A front door) — from the packaged `agents.sh` (wired by `synapse install`) —
do this and launch the session for you. Discovery: `synapse agents` · `synapse hubs` · `synapse help`.
...
`rules/rule-canary.md`, the canary trailer in `synapse render`, and this file.)
