...
When asked to act as a named agent (e.g. "act as agent-curator", "run the reconciler on moc-finances"),
**render its bundled briefing** instead of opening files one by one:

```
node _meta/tools/render.mjs <agent-id> [<target-id>] --profile <lean|standard|fat>
```
...
`rules/rule-canary.md`, the `CANARY_TRAILER` in `_meta/tools/render.mjs`, and this file.)
