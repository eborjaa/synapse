 ...
 *   newestPath: string|null, storedMaxMtime: string|null, collisions: number|null, dbPath: string,
 *   reason: string
 * }}
 ...
 */
export function embeddingsStatus({ paths = freshnessPaths(), precise = true, deep = false } = {}) {
  const base = {
    ...
    storedMaxMtime: null, collisions: null, dbPath: paths.dbPath, reason: "",
  };
  ...
  // The id-collision count is only produced by the exact pass. Carry the last known value on the cheap
  // paths so the warning does not blink in and out depending on which tier happened to answer.
  const marker = readMarker(paths.markerPath);
  base.collisions = marker?.collisions ?? null;

  // ---- a model change invalidates EVERY vector, and no amount of mtime comparison can see it: the
  // files are untouched, only the embedding space moved. Cosine across two models is meaningless, so
  // this is checked before the time-based tiers and short-circuits straight to "rebuild everything".
  const wantModel = safeEmbedModel();
  if (wantModel && model && model !== wantModel) {
    return {
      ...base, stale: true, staleCount: rows, precise: true,
      reason: `index was built with "${model}" but the configured model is "${wantModel}" — a full re-embed is required`,
    };
  }

  // `deep` skips both cheap short-circuits. augment leaves it off — it wants the fastest honest answer
  // on a hot path. An explicit `embeddings-status` turns it on: someone ASKED, so pay the 400ms and
  // report everything the exact pass knows, including id collisions (which the short-circuits cannot
  // see, since a vault can be perfectly current AND be silently shadowing notes).
  // ---- tier 1: no file newer than the newest indexed note → provably nothing to do.
  // Compare in the EXACT form gen-embeddings stores (`stat().mtime.toISOString()`) rather than raw
  // epoch-ms. Sub-millisecond precision survives in mtimeMs but not in the stored string, and the two
  // do not round the same way — comparing across the formats flips the verdict on a sub-ms difference.
  // ISO-8601 UTC strings of equal shape compare lexicographically in chronological order.
  const newestIso = isoMtime(newest.path);
  if (!deep && storedMax && newestIso && newestIso <= storedMax) {
  ...
  //
  // De-duplicate by id FIRST, exactly the way gen-embeddings does. Note ids are basenames and are
  // global, so when two files share one (plans/alerts/step-01.md and plans/cases/step-01.md) only the
  // last one walked is embedded — one id, one row. Comparing every FILE against that single row marks
  // the losers stale forever: no rebuild can ever clear them, so the self-heal would fire on every call
  // and do nothing, which is the exact loop this module exists to prevent. Same walker, same last-wins
  // rule, same verdict. The collisions are still counted and reported — the losing notes are genuinely
  // absent from recall, which is a vault problem worth surfacing, just not a freshness one.
  const embedModel = wantModel;
  const byId = new Map();
  let collisions = 0;
  for (const f of files) {
    ...
    if (byId.has(id)) collisions++;
    byId.set(id, mtime);
  }
  base.collisions = collisions;

  let staleCount = 0;
  for (const [id, mtime] of byId) {
  ...
  if (!staleCount) {
    ...
    writeMarker(paths.markerPath, newest.ms, collisions);
...
/** A file's mtime in the exact string form gen-embeddings persists, or null if it vanished. */
function isoMtime(path) {
  try { return path ? statSync(path).mtime.toISOString() : null; } catch { return null; }
}
...
function readMarker(path) {
  try {
    ...
    return { ms: j.clearedCorpusMaxMtimeMs ?? null, collisions: j.collisions ?? null };
  } catch { return null; }
}

function writeMarker(path, ms, collisions = null) {
  try {
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, JSON.stringify({
      clearedCorpusMaxMtimeMs: ms, collisions, at: new Date().toISOString(),
    }), "utf8");
  ...
  try {
    ...
    if (r.status === 0) {
      ...
      writeMarker(paths.markerPath, maxMtime(walkNoteFiles(paths.root, noteSkipSet(paths.manifest))).ms,
        st.collisions ?? readMarker(paths.markerPath)?.collisions ?? null);
...
export function formatStatus(st) {
  ...
  const dup = st.collisions > 0
    ? ` · ⚠ ${st.collisions} note(s) share an id with another and are NOT indexed (rename to recover them)`
    : "";
  return `embeddings: ${head} — ${st.reason} `
    + `(indexed ${st.rows}, corpus ${st.corpusNotes}, model ${st.model || "?"}${st.precise ? "" : ", fast check"})${dup}`;
}
...
if (IS_MAIN) {
  ...
  let st = embeddingsStatus({ paths, precise: !fast, deep: !fast });
  let refresh = null;
  if (wantRefresh) {
    refresh = refreshIfStale({ paths, force, all, status: force ? null : st, log: (m) => console.error(m) });
    if (refresh.refreshed) st = embeddingsStatus({ paths, precise: true, deep: true });
  }
