...
//
// ROOT = the vault root (resolved via vault-root.mjs). Determinism: same vault → same rows.
...
const ROLE_ENTRIES = Object.entries(MANIFEST.roles || {}); // [name, {field|fields, endpointTypes?, …}]
function resolveRole(field, dstType) {
  const usingField = ROLE_ENTRIES.filter(([, r]) => roleFieldsOf(r).includes(field));
