...
// opencode `run` is ONE-SHOT by default and exits after answering; --interactive is what keeps the
// session open. It was removed once by mistake (on a wrong reading of the flag list) and the TUI
// silently stopped appearing — the run still "worked", which is why only a human noticed.
test("[cli:opencode] --interactive is passed so the session STAYS OPEN (TUI)", () => {
  const a = launchArgv("opencode", `"fix the grid"`);
  assert.match(a, /\[--interactive\]/, "opencode run is one-shot without --interactive");
  assert.match(a, /\[--dir\] \[[^\]]+\]/, "the vault dir is passed so tools operate in the vault");
});
