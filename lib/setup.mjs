...
// ── build the index ─────────────────────────────────────────────────────────
// A provisioned runtime with no index is the trap this step closes: `setup` used to end at "✅ GO" while
// db/synapse.db did not exist, so every augment silently fell back to the deterministic spine. Only
// meaningful once Ollama + the model are actually there, so it is gated on `go`.
const go = !!tags && modelPresent;
if (go) {
  console.log(`\n📇 embedding index`);
  const { embeddingsStatus, refreshIfStale, formatStatus } = await import("./index-freshness.mjs");
  const status = embeddingsStatus({ precise: true });
  console.log(`  ${formatStatus(status)}`);
  if (!status.stale) ok("nothing to build");
  else if (await confirm(`Build it now (${status.staleCount ?? "?"} note(s), a few minutes on first run)?`)) {
    const res = refreshIfStale({ status, force: true, log: (m) => console.log(`    ${m}`) });
    if (res.refreshed) ok(`index built — ${res.summary}`);
    else no(`index not built (${res.reason}) — run 'synapse embeddings' manually`);
  } else {
    console.log(`→ To build it later:`);
    info(`synapse embeddings          (status anytime: synapse embeddings-status)`);
  }
}
