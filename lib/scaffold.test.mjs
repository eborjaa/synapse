...
// `addressable: true` is the flag an OPERATOR (Cortex) scans for to build its run roster. Omitting it
// produces a note that lints clean and renders fine but is never started, so the default must stay
// off — and the opt-in must actually appear in the frontmatter, not just in the tool's description.
test("build agent: addressable is opt-in and lands in the frontmatter", () => {
  const on = build({ kind: "agent", slug: "runner", purpose: "x", addressable: true, manifest });
  assert.match(fm(on.content), /^addressable: true$/m);

  const off = build({ kind: "agent", slug: "runner", purpose: "x", manifest });
  assert.doesNotMatch(fm(off.content), /^addressable:/m);

  // Non-agent types never carry the flag, even if a caller passes it.
  const note = build({ kind: "note", slug: "x", type: "rule", addressable: true, manifest });
  assert.doesNotMatch(fm(note.content), /^addressable:/m);
});
