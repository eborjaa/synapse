...
// ── on-demand: adversarial edge cases (probed 2026-08-19; EDGE 2 caught a real chained-reference bug) ──
// A dedicated manifest whose ATTACHES/NAVIGATES/BINDS all read `related`, matching a real vault shape.
const OD_MANIFEST = (() => {
  const m = structuredClone(MANIFEST);
  m.tokenBudgets = { lean: 15000, standard: 15000, fat: 15000 };
  return m;
})();
function odVault(files) {
  const root = mkdtempSync(join(tmpdir(), "syn-odx-"));
  mkdirSync(join(root, "_meta", "tools"), { recursive: true });
  writeFileSync(join(root, "_meta", "tools", "context.manifest.json"), JSON.stringify(OD_MANIFEST));
  for (const [rel, content] of Object.entries(files)) {
    const p = join(root, rel); mkdirSync(join(p, ".."), { recursive: true }); writeFileSync(p, content);
  }
  return { root, cleanup: () => rmSync(root, { recursive: true, force: true }) };
}

test("[on-demand] EDGE: referenced from a HUB, not a rule", () => {
  const v = odVault({
    "agents/agent-a.md": note("agent-a", "agent", "purpose: x\nrelated: [[hub-m]]\n"),
    "hub/hub-m.md": note("hub-m", "hub", "related: [[glossary-g]]\n"),
    "glossary/glossary-g.md": note("glossary-g", "glossary", 'on_demand: true\ntrigger: "when H applies"\n', "BODY " + "x".repeat(5000)),
  });
  try {
    const { stdout } = render(v.root, ["agent-a", "hub-m", "--profile", "standard"]);
    assert.match(stdout, /when H applies/);
    assert.doesNotMatch(stdout, /BODY xxxx/);
  } finally { v.cleanup(); }
});

test("[on-demand] EDGE: CHAINED — an on-demand note referencing another surfaces BOTH triggers", () => {
  const v = odVault({
    "agents/agent-a.md": note("agent-a", "agent", "purpose: x\napplies_rules: [[rule-r]]\n"),
    "rules/rule-r.md": note("rule-r", "rule", "related: [[glossary-a]]\n", "rule body"),
    "glossary/glossary-a.md": note("glossary-a", "glossary", 'on_demand: true\ntrigger: "trigger A"\nrelated: [[glossary-b]]\n', "A body"),
    "glossary/glossary-b.md": note("glossary-b", "glossary", 'on_demand: true\ntrigger: "trigger B"\n', "B body"),
  });
  try {
    const { stdout } = render(v.root, ["agent-a", "--profile", "standard"]);
    assert.match(stdout, /trigger A/, "A, referenced by the rule");
    assert.match(stdout, /trigger B/, "B, referenced only by on-demand A — the chain must resolve");
  } finally { v.cleanup(); }
});

test("[on-demand] EDGE: no trigger field degrades to the id, body still suppressed", () => {
  const v = odVault({
    "agents/agent-a.md": note("agent-a", "agent", "purpose: x\napplies_rules: [[rule-r]]\n"),
    "rules/rule-r.md": note("rule-r", "rule", "related: [[glossary-g]]\n", "rb"),
    "glossary/glossary-g.md": note("glossary-g", "glossary", "on_demand: true\n", "BODY " + "x".repeat(5000)),
  });
  try {
    const { stdout } = render(v.root, ["agent-a", "--profile", "standard"]);
    assert.doesNotMatch(stdout, /BODY xxxx/);
    assert.match(stdout, /glossary-g/);
  } finally { v.cleanup(); }
});

test("[on-demand] EDGE: the AUGMENT path (how agents actually brief) keeps the trigger", () => {
  const v = odVault({
    "agents/agent-a.md": note("agent-a", "agent", "purpose: x\napplies_rules: [[rule-r]]\n"),
    "rules/rule-r.md": note("rule-r", "rule", "related: [[glossary-g]]\n", "rb"),
    "glossary/glossary-g.md": note("glossary-g", "glossary", 'on_demand: true\ntrigger: "AUG trigger"\n', "BODY " + "x".repeat(5000)),
  ...
  try {
    const AUGMENT = join(dirname(fileURLToPath(import.meta.url)), "augment.mjs");
    const r = spawnSync(process.execPath, [AUGMENT, "agent-a", "--profile", "standard", "--no-semantic"],
      { cwd: v.root, encoding: "utf8", env: { ...process.env, VAULT_USER: "t", SYNAPSE_VAULT: v.root }, maxBuffer: 64e6 });
    assert.match(r.stdout, /AUG trigger/, "augment shells to render and must preserve on-demand");
    assert.doesNotMatch(r.stdout, /BODY xxxx/);
  } finally { v.cleanup(); }
});
