...
//
// synapse ships as an npm package (@eborjaa/synapse). Its tools therefore live in
// node_modules/@eborjaa/synapse/, NOT inside the consumer's vault — so the legacy "N-up from
...
export function resolveVault({ cwd = process.cwd(), readManifest = true } = {}) {
  const hit = process.env.SYNAPSE_VAULT ? resolveOverride(process.env.SYNAPSE_VAULT) : walkUp(cwd);
  if (!hit) {
    throw new Error(
