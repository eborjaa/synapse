...
//
// preferCwd (DEFAULT true): walk from cwd first, fall back to $SYNAPSE_VAULT only when cwd is not
// inside a vault. This matches how a human uses the CLI and the launcher (agents.sh __mx_vault) — you
// `cd` into a vault and expect commands to brief from THERE, and a stale exported $SYNAPSE_VAULT must
// NOT silently override that (the exact trap that made a render read the wrong vault: an rc-exported
// override winning over the dir you were standing in).
//
// Pass preferCwd:false ONLY for a config-PINNED, long-lived process that cannot `cd` — the MCP server,
// launched by the harness with $SYNAPSE_VAULT set in .mcp.json. There the env IS the authoritative
// intent, so it must win. Every interactive tool wants the default.
export function resolveVault({ cwd = process.cwd(), readManifest = true, preferCwd = true } = {}) {
  let hit = null;
  if (preferCwd) {
