...
async function main() {
  ...
  // The walk is SHARED with index-freshness.mjs (lib/note-walk.mjs): dotdirs + manifest.skipDirs, plus a
  // hard skip of node_modules/db. If the two walked different sets, freshness would compare this index
  // against a corpus it does not describe. The hard skip also stops a vendored package that ships its own
  // typed notes (this one ships an example vault) from being embedded into a consumer's index.
  const SKIP = noteSkipSet(MANIFEST);

  // index every typed note (.md with a type: field)
  const notes = new Map(); // id → { title, body, mtime }
  for (const f of walkNoteFiles(WALK_ROOT, SKIP)) {
