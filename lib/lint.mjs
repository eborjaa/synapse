...
import { scanText as scanLivenessAntiPattern } from "./durable-spawn/lint.mjs";
import { join, basename } from "node:path";
import { execFileSync } from "node:child_process";
import { resolveVault } from "./vault-root.mjs";
import { walkNoteFiles, noteSkipSet } from "./note-walk.mjs";
import { PREFIX_TYPE, REQUIRED } from "./schema.mjs";

const { root: ROOT, manifest: MANIFEST } = resolveVault();
// Same walker as render/gen-embeddings (lib/note-walk.mjs): manifest.skipDirs PLUS a hard skip of
// node_modules and db. Without it a consumer lints the example vault this package ships inside its own
// node_modules — 148 phantom "typed note is git-ignored" errors in a vault whose own notes are fine.
const SKIP = noteSkipSet(MANIFEST);
const LABEL = MANIFEST.logLabel || "synapse";
const strict = process.argv.includes("--strict");

// PREFIX_TYPE (prefix → `type:`) and REQUIRED (per-type frontmatter) now live in ./schema.mjs, shared
// with the scaffolders so the checker and the generator can never disagree.
//
...
const allFiles = walkNoteFiles(ROOT, SKIP);
...
for (const f of noteFiles) {
  ...
  // 5b. durable-spawn anti-pattern — a briefing/rule must never tell an orchestrator to judge a doer's
  //     liveness by a file's modified time (incident a9463a07). Enforced in code so it can't creep back
  //     via a prompt edit; per-line `<!-- durable-spawn-lint: allow -->` markers exempt legit discussion.
  for (const hit of scanLivenessAntiPattern(text))
    errors.push(`${id}:${hit.lineNo}: durable-spawn anti-pattern (${hit.id}): ${hit.text}`);
