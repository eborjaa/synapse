...
function parseNote(raw) {
  ...
  // ON-DEMAND notes carry a TRIGGER instead of a body: a rule whose payload is a template or a
  // procedure is only needed at the instant it applies, and carrying it always costs more context than
  // everything else the agent knows. The trigger is short precisely because it names a SITUATION, not
  // the procedure. See rule-on-demand-context.
  const onDemand = /^on_demand:\s*true\s*$/m.test(fm);
  const trigger = (fm.match(/^trigger:\s*"?(.+?)"?\s*$/m) || [])[1] || "";
  const fields = {};
  for (const f of ROLE_FIELDS) fields[f] = linksInField(fm, f);
  return { type, tags, body, sp, fields, onDemand, trigger };
}

const index = new Map(); // basename → { type, tags, body, sp, fields, onDemand, trigger }
...
function closure(startIds, prof) {
  ...
  // ON-DEMAND STICKINESS: an on-demand note referenced by ANY note already in the closure joins it —
  // regardless of hop distance or profile depth. Its body never renders (renderBody stubs it to a
  // ~35-token trigger), so depth-gating it only produces dangling pointers: a rule that reached the
  // briefing and says "fetch this template before writing" must have its target's trigger reach the
  // agent too. Only the reference-typed fields are followed (the same edges ATTACHES/NAVIGATES use), so
  // this never drags in members. The role label is the referencing note's own role, so it inherits a
  // sensible ordering slot.
  // The fields to follow are every field any ENABLED role in this profile reads — derived from the
  // profile, never from an optional manifest key (referenceRoles is advisory and many vaults omit it).
  // We follow a note's own OUTBOUND links (rec.fields is the frontmatter it declares), so this pulls
  // "X says fetch Y", never members pointing back at a hub.
  const refFields = new Set();
  for (const rn of pcfg.roles || []) {
    const role = MANIFEST.roles[rn]; if (!role) continue;
    for (const f of roleFieldsOf(role)) refFields.add(f);
  }
  // To a FIXPOINT, not a single snapshot pass: an on-demand note may itself reference another on-demand
  // note (a checklist pointing at a template), and a one-shot loop would miss the second link because
  // the first was added mid-iteration. A worklist resolves the whole chain; the `seen` guard bounds it.
  let frontier = [...seen];
  while (frontier.length) {
    const next = [];
    for (const n of frontier) {
      const rec = index.get(n); if (!rec) continue;
      for (const f of refFields) for (const c of (rec.fields[f] || [])) {
        const cr = index.get(c);
        if (cr && cr.onDemand && !seen.has(c) && !drop(c)) {
          seen.add(c); addedBy.set(c, addedBy.get(n) || (pcfg.roles || [])[0] || "USES"); next.push(c);
        }
      }
    }
    frontier = next;
...
// An on-demand note is a POINTER unless it was explicitly asked for — asking for it by id is how you
// read it. This deliberately outranks mandatoryFull: "always included" and "always inlined" are
// different claims, and a template that must be followed exactly is best read fresh, not recalled.
const isOnDemand = (nid) => !isStart(nid) && !!index.get(nid)?.onDemand;
const isMandatory = (nid) => isStart(nid) || isOnDemand(nid) || (CONSTRAIN_ROLE && addedBy.get(nid) === CONSTRAIN_ROLE);
function renderBody(nid) {
  const rec = index.get(nid);
  if (isOnDemand(nid)) {
    return `**NOT LOADED — fetch this before you act.** Applies: ${rec.trigger || "see the note"}\n\n`
      + `Read it FIRST with \`synapse_brief(note: "${nid}")\` (or \`synapse render ${nid}\`). `
      + `Do not improvise or paraphrase it from memory.`;
  }
...
// One checklist of every on-demand note, so the triggers read as a group rather than as stubs
// scattered through the briefing. This is the part an agent scans when deciding "does this apply now?".
const onDemandNodes = finalNodes.filter((r) => isOnDemand(r.nid));
const onDemandIndex = onDemandNodes.length
  ? `\n\n<!-- fetch-before-you-act -->\n## Fetch before you act\n\n`
    + `These notes are NOT loaded — only their triggers are. When one applies, read it before acting.\n\n`
    + onDemandNodes.map((r) => `- **${index.get(r.nid).trigger || r.nid}** → \`synapse_brief(note: "${r.nid}")\``).join("\n")
    + `\n`
  : "";
const trailers = buildTrailers(MANIFEST, { root: ROOT });
const out = body + onDemandIndex + "\n" + trailers;
