...
// ── suite affinity: when a task NAMES a suite, recall biases toward that suite's notes ──────────────
// (The semantic BOOST needs an index+Ollama, verified live against rel-context-eb: an "alerts" task
//  that used to return sensors-notification notes now returns workflow-alerts-*/user-story-alerts-*.
//  Here we test the deterministic routing DECISION — suitesNamedBy — which drives the boost.)
import { suitesNamedBy } from "./recall.mjs";

function suiteVault() {
  return vault({
    "suites/alerts/moc-alerts.md": note("moc-alerts", "moc", "tags:\n  - suite/alerts\n"),
    "suites/sensors/moc-sensors.md": note("moc-sensors", "moc", "tags:\n  - suite/sensors\n"),
    "suites/work-orders/moc-wo.md": note("moc-wo", "moc", "tags:\n  - suite/work-orders\n"),
    "suites/spare-parts/moc-sp.md": note("moc-sp", "moc", "tags:\n  - suite/spare-parts\n"),
  });
}

test("suitesNamedBy: a task naming one suite routes to exactly that suite", () => {
  const v = suiteVault();
  try {
    assert.deepEqual([...suitesNamedBy(v.ctx, "the alerts fanout list is broken")], ["alerts"]);
    assert.deepEqual([...suitesNamedBy(v.ctx, "sensors grid layout")], ["sensors"]);
  } finally { v.cleanup(); }
});

test("suitesNamedBy: a MULTI-WORD suite matches only when both parts are present", () => {
  const v = suiteVault();
  try {
    assert.deepEqual([...suitesNamedBy(v.ctx, "close-verify the work order bugs")], ["work-orders"], "work + order → work-orders");
    assert.deepEqual([...suitesNamedBy(v.ctx, "just the orders screen")].includes("work-orders"), false, "one part is not enough");
    assert.deepEqual([...suitesNamedBy(v.ctx, "spare parts inventory")], ["spare-parts"]);
  } finally { v.cleanup(); }
});

test("suitesNamedBy: a task naming NO suite routes nowhere (the boost stays off)", () => {
  const v = suiteVault();
  try {
    assert.equal(suitesNamedBy(v.ctx, "fix the flaky login timeout").size, 0);
  } finally { v.cleanup(); }
});

test("recall exposes routedToSuites so a wrong route is visible", async () => {
  const v = suiteVault();
  try {
    const r = await recall({ vault: v.ctx, task: "the alerts list is empty" });
    assert.deepEqual(r.routedToSuites, ["alerts"]);
  } finally { v.cleanup(); }
});
