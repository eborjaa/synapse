...
import { readRegistry, skillsRoot } from "./vaults.mjs";

/** Public vault list on the skills volume — id + root only, never tokens. DSH reads this. */
export function vaultIndexPath() {
  return process.env.SYNAPSE_VAULT_INDEX || join(skillsRoot(), "index.json");
}

/**
 * Registry if this process has one; otherwise the public index core wrote for DSH.
 * An empty host registry must not hide a populated index (the container DSH case).
 */
export function readVaultDirectory() {
  let fromHome = { version: 1, vaults: [] };
  try { fromHome = readRegistry(); } catch { /* no SYNAPSE_HOME in DSH */ }
  if (fromHome.vaults?.length) return fromHome;
  const p = vaultIndexPath();
  if (!existsSync(p)) return fromHome;
  try {
    const raw = JSON.parse(readFileSync(p, "utf8"));
    if (raw && Array.isArray(raw.vaults)) return { version: raw.version || 1, vaults: raw.vaults };
  } catch { /* fall through */ }
  return fromHome;
}
...
export function vaultForCwd(cwd, { reg = null } = {}) {
  ...
  const registry = reg || readVaultDirectory();
