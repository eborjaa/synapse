...
test("every registered adapter satisfies its own port", () => {
  for (const reg of [clientConfigAdapters, rosterAdapters, toolTransportAdapters, vaultBindingAdapters, vaultStoreAdapters]) {
    for (const a of reg.all()) assert.doesNotThrow(() => reg.port.assert(a), `${reg.port.name}/${a.id}`);
  }
});

test("VaultStorePort now HAS an adapter — stage 4 landed", () => {
  // This test previously asserted the port was declared-only. Stage 4 implemented it, so the assertion
  // flipped rather than being deleted: the point is that the transition is visible in the diff.
  assert.ok(vaultStoreAdapters.all().length > 0, "stage 4 implements this port");
