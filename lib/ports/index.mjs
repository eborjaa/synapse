...
const httpTransport = {
   ...
   * calls the SAME buildServer({ surface, plugins, vault }) used by stdio.
   */
  async serve(buildServer, {
    ...
    const handler = createMcpHandler((ctx) => {
      // `ctx.authInfo`, never MCP params, is the only value handed to the binding. Passing the whole
      // Request here would make it possible for a future adapter to "helpfully" inspect tool arguments.
      const bound = binding.bind({ authInfo: ctx.authInfo });
      ...
      vault.assertVault();
      return buildServer({ surface, plugins, vault });
    }, {
    ...
    const server = createServer((req, res) => {
      void (async () => {
        ...
        const authInfo = token
          ...
          : undefined;
        const preflight = binding.bind({ authInfo });
        if (!preflight.ok) {
          req.resume();
