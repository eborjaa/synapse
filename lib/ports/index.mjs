...
//   ToolTransportPort  ◻︎ stdio only — but no longer BLOCKED. This entry used to record why: the HTTP
//                        adapter was never an SDK gap (createMcpHandler ships in
//                        @modelcontextprotocol/server@2), it was that mcp/vault.mjs resolved VAULT once
//                        at module load and ~60 references across 8 tool modules read that constant, so
//                        an HTTP server would have accepted a per-request credential, resolved it
//                        correctly, and then served every request from whichever vault loaded first.
//                        That threading is DONE: a bound vault is a value (mcp/vault-context.mjs) handed
//                        to buildServer({ vault }) and closed over by each handler. What is left is the
//                        adapter itself, and its contract test already exists in two halves — the tool
//                        list must not change with the vault (mcp/vault-context.test.mjs) nor with the
//                        transport (here).
...
export { bearerVaultBinding, bearerBindingAdapters } from "./vault-tokens.mjs";

/** Every port declared in this package, for diagnostics and for the contract-test sweep. */
export const ALL_PORTS = Object.freeze([
  ClientConfigPort, RosterPort, ToolTransportPort, VaultBindingPort, _VaultStorePort,
]);
