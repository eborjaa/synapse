...
//   ToolTransportPort  ◻︎ stdio only. The HTTP adapter is NOT blocked on the SDK — createMcpHandler
//                        ships in @modelcontextprotocol/server@2. It is blocked on one thing, stated
//                        here so nobody rediscovers it: mcp/vault.mjs resolves VAULT ONCE at module
//                        load, and 67 references across 8 tool modules read that constant. Until the
//                        bound vault is threaded through those call sites, an HTTP server would accept
//                        a per-request credential, resolve it correctly, and then serve every request
//                        from whichever vault happened to load first — multi-vault in the URL and
//                        single-vault in the data. That is strictly worse than no HTTP at all, because
//                        it looks like it works. Thread the vault first; then add the transport.
