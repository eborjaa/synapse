...
export function buildDoerArgv({ cli, briefingFile, vault, model, permMode = "auto", cwd }) {
  ...
  switch (cli) {
    ...
    case "opencode": {
      // `--file` is an ARRAY flag, so a boolean flag (`--auto`, which also auto-approves permissions for
      // the non-interactive doer) must follow it to terminate the array before the trailing message.
      const args = ["run", ...(model ? ["-m", model] : []), "--dir", vault, "--file", briefingFile, "--auto"];
...
export function launchDetached({
  ...
  // 0. The doer's emit command is a real EXECUTABLE (not a multi-word string): the doer runs it as
  //    `$SYNAPSE_SPAWN_EMIT HEARTBEAT …`, and zsh (opencode's shell) does NOT word-split an unquoted
  //    variable, so a "node <bin> spawn-emit" string would be treated as one command name. A single
  //    script sidesteps every shell's splitting rules.
  const emitScript = join(runDir, "emit");
  writeFileSync(emitScript, `#!/bin/sh\nexec ${shq(process.execPath)} ${shq(SYNAPSE_BIN)} spawn-emit "$@"\n`, "utf8");
  chmodSync(emitScript, 0o755);
  ...
  const exports = [
    ...
    // The doer reports progress via $SYNAPSE_SPAWN_EMIT — a single executable script (above), so it
    // resolves from any shell (no word-splitting) and never depends on a `synapse` alias being loaded.
    + `\nexport SYNAPSE_SPAWN_EMIT=${shq(emitScript)}`;
