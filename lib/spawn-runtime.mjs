...
export function heartbeatPreamble() {
  return [
    ...
    "apart from a hang — run these as shell commands (owner/token/status ride in your environment):",
    "",
    "- Every few minutes, or when you finish a step:  `synapse spawn-emit HEARTBEAT <stage> <metric>`",
    "  (the orchestrator renews your claim when it sees this — go quiet too long and it will flag a hang).",
    "- Before a long, quiet operation (a test run, a build):  `synapse spawn-emit WAITING <what> <etaMinutes>`",
    "- On success:  `synapse spawn-emit DONE <one-line summary>`   ·   On failure:  `synapse spawn-emit FAILED <reason>`",
    "",
 ...
 */
export function buildDoerArgv({ cli, briefingFile, task, vault, model, permMode = "auto", cwd }) {
  const perm = (map) => (map[permMode] ? map[permMode] : []);
  switch (cli) {
    case "claude": {
      const args = [
        ...perm({ auto: ["--permission-mode", "auto"], bypass: ["--permission-mode", "bypassPermissions"], manual: [] }),
        "-p", // headless / print — run the agent loop non-interactively, then exit
        "--append-system-prompt-file", briefingFile,
        "--add-dir", vault,
        ...(model ? ["--model", model] : []),
        ...(task ? [task] : []),
      ];
      return { bin: "claude", args, prep: null };
    }
    case "cursor": {
      const rulesFile = join(cwd || vault, ".cursor", "rules", ".synapse-spawn-briefing.mdc");
      const args = [
        "-p", // print / non-interactive
        ...perm({ auto: ["--auto-review"], bypass: ["--force"], manual: [] }),
        ...(model ? ["--model", model] : []),
        "--add-dir", vault,
        ...(task ? [task] : []),
      ];
      return { bin: "cursor-agent", args, prep: { file: rulesFile, briefingFile, framed: true } };
    }
    case "opencode": {
      const args = ["run", ...(model ? ["-m", model] : []), "--dir", vault, "--file", briefingFile, ...(task ? [task] : [])];
      return { bin: "opencode", args, prep: null };
    }
    default:
...
export function launchDetached({
  ...
  // 2. Per-CLI argv + prep.
  const { bin, args, prep } = buildDoerArgv({ cli, briefingFile, task, vault, model, permMode, cwd });
  if (prep?.file) {
  ...
  const runtimeCmd = [shq(bin), ...args.map(shq)].join(" ");
