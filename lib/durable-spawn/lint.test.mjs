...
test("does NOT flag ordinary prose where soft time + liveness words co-occur without a file", () => {
  // Regression: a Zephyr test-case row "stale case + dead selector" tripped the old heuristic because
  // `stale` (time) sat near `dead` (liveness). No transcript, no file-time word → not the anti-pattern.
  assert.deepEqual(scanText("| T34618 | Blocked | — (stale case + dead selector) | check only |"), []);
  assert.deepEqual(scanText("The fixture is stale and the old button is dead code now."), []);
});
