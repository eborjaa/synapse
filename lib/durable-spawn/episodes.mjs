...
//
//   2. PRIMARY DATA, IN ITS OWN FILE. Never db/synapse.db — that is a rebuildable embeddings cache any
//      `synapse embeddings --all` may discard. And deliberately NOT db/durable-spawn.db either, which
//      holds leases: the obvious fix for a stuck lock is "delete that file", and mixing throwaway
//      runtime state with permanent memory in one file invites exactly that loss. db/episodes.db is
//      safe to leave alone and safe to back up on its own.
//
...
/** Open (and migrate) the episode store. Its own file — see note 2 above. */
...
export function migrate(db) {
  db.exec(`CREATE TABLE IF NOT EXISTS episode(
    ...
    outcome     TEXT,                 -- open | done | failed | abandoned | ended-unknown
    ...
    ended_at    INTEGER,
    closed_at   INTEGER,              -- set when outcome leaves 'open' (invariant with outcome)
    handle      TEXT                  -- checksummed handoff handle (decision-0019)
  );`);
  try { db.exec("ALTER TABLE episode ADD COLUMN closed_at INTEGER;"); } catch { /* already present */ }
  try { db.exec("ALTER TABLE episode ADD COLUMN handle TEXT;"); } catch { /* already present */ }
  ...
  db.exec("CREATE UNIQUE INDEX IF NOT EXISTS episode_handle ON episode(handle) WHERE handle IS NOT NULL;");
 ...
 */
export function open(db, { agent = null, job = null, spawnId = null, task, hub = null, branch = null, handle = null }, now) {
  if (!task || !String(task).trim()) throw new Error("episodes.open: task is required");
  const at = now ?? dbNow(db);
  const episodeId = randomUUID();
  db.prepare(
    `INSERT INTO episode (episode_id, agent, job, spawn_id, task, hub, branch, outcome, started_at, handle)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'open', ?, ?)`,
  ).run(episodeId, agent, job, spawnId, String(task), hub, branch, at, handle);
...
export function close(db, { episodeId = null, job = null, outcome = "done", summary = null, refs = null }, now) {
  ...
  const closedAt = at;
  db.prepare("UPDATE episode SET outcome = ?, summary = COALESCE(?, summary), refs = ?, ended_at = ?, closed_at = ? WHERE rowid = ?")
    .run(outcome, summary, refsJson, at, closedAt, target.rowid);
...
function shape(r) {
  ...
  return {
    ...
    refs, startedAt: r.started_at, endedAt: r.ended_at, closedAt: r.closed_at,
