...
import { openEpisodeDb, open, close, log, searchEpisodes, lastForJob, reindex } from "./episodes.mjs";

function db() {
  const dir = mkdtempSync(join(tmpdir(), "syn-ep-"));
  const d = openEpisodeDb(join(dir, "episodes.db"));
...
test("episodes survive reopening the database (they are primary data, not a cache)", () => {
  ...
  try {
    const p = join(dir, "episodes.db");
    const a = openEpisodeDb(p);
    log(a, { job: "j9", task: "persist me", summary: "kept" });
    a.close();
    const b = openEpisodeDb(p);
