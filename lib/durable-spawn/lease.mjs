...
export function openDb(path) {
  ...
  try { db.exec("ALTER TABLE lease ADD COLUMN handle TEXT;"); } catch { /* already present */ }
  db.exec("CREATE UNIQUE INDEX IF NOT EXISTS lease_handle ON lease(handle) WHERE handle IS NOT NULL;");
 ...
 */
export function acquire(db, job, owner, ttlMs, now, extra = {}) {
  if (!(ttlMs > 0)) return { ok: false, reason: "bad-ttl" };
  db.exec("BEGIN IMMEDIATE");
  try {
    const at = now ?? dbNow(db);
    const cur = db
      .prepare("SELECT owner, token, ttl_ms, renewed_at, handle FROM lease WHERE job = ?")
      .get(job);
    if (isLive(cur, at)) {
      db.exec("ROLLBACK");
    ...
    const handle = extra.handle ?? null;
    db.prepare(
      `INSERT INTO lease(job, owner, token, ttl_ms, renewed_at, handle) VALUES (?,?,?,?,?,?)
       ON CONFLICT(job) DO UPDATE SET
         owner = excluded.owner, token = excluded.token,
         ttl_ms = excluded.ttl_ms, renewed_at = excluded.renewed_at,
         handle = excluded.handle`,
    ).run(job, owner, token, ttlMs, at, handle);
    db.exec("COMMIT");
