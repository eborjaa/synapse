...
export function migrate(db) {
  ...
  // Forward-compat: a spawn table created before `task` existed gets the column added.
...
export function record(db, { spawnId, job, owner, epoch, token, statusFile, task }, now) {
  if (!epoch) throw new Error("record: epoch is required (per-boot id)");
  db.prepare(
    `INSERT INTO spawn(spawn_id, job, owner, epoch, token, status_file, task, state, created_at, updated_at)
     VALUES (?,?,?,?,?,?,?, 'running', ?, ?)
  ...
  ).run(spawnId, job, owner, epoch, token ?? null, statusFile ?? null, task ?? null, now, now);
}
