...
kinds:
  ...
  agent <id>          a new agent                 --purpose "…" [--rules a,b] [--tools a,b]
                                                  ...
                                                  [--addressable]  make it a STANDING agent an
                                                  operator will run; without it the note is a
                                                  persona that nothing ever starts
...
try {
  out = build({
    ...
    plan: flag("plan"),
    addressable: argv.includes("--addressable"),
    manifest,
  });
} catch (err) {
