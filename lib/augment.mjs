...
import { embeddingsStatus, refreshIfStale, freshnessPaths } from "./index-freshness.mjs";
...
const positional = [];
let profile = null, task = null, k = DEFAULT_K, noSemantic = false, handoverRef = null, promptFile = null;
for (let i = 0; i < argv.length; i++) {
  const a = argv[i];
  if (a === "--profile") { profile = argv[++i]; continue; }
  if (a === "--task") { task = argv[++i]; continue; }
  if (a === "--handover") { handoverRef = argv[++i]; continue; }   // a note (a handover) IS the task
  if (a === "--prompt-file") { promptFile = argv[++i]; continue; } // any note as the task, no protocol
...
// --handover / --prompt-file resolve a NOTE into the task (the "boot from a handover" primitive). The
// note text becomes both the appended task and augment's recall query — the same shape as --task.
if ((handoverRef || promptFile) && !task) {
  const { noteAsTask } = await import("./note-as-task.mjs");
  const ref = handoverRef || promptFile;
  const r = noteAsTask(ref, { vaultDir: VAULT_DIR, handover: !!handoverRef });
  if (!r.ok) { console.error(`[${LABEL} augment] --${handoverRef ? "handover" : "prompt-file"}: ${r.reason}`); process.exit(2); }
  task = r.task;
}
...
// ---- index freshness (in-band, never fatal) --------------------------------------------------------
// A MISSING index has always been loud (the skip note below); a STALE one used to be silent — recall
// ranked against the vault as it was and said nothing. FRESHNESS carries the verdict so every exit path
// (skip note AND a successful augment) can state it in-band, next to the results it affects.
let FRESHNESS = null;
function freshnessNote() {
  const st = FRESHNESS?.status;
  if (!st?.stale || FRESHNESS.refreshed) return "";
  const behind = st.staleCount == null ? "behind the vault" : `${st.staleCount} note(s) behind the vault`;
  return `> ⚠ semantic index is ${behind} — run \`synapse embeddings\` (${FRESHNESS.reason}).\n`;
}
...
async function augment() {
  // Self-heal a stale/absent index BEFORE reading it, so recall reflects the vault as it is now. This
  // shells out to gen-embeddings as a CHILD process — the invariant below (this path never mutates the
  // DB) still holds for this process. Best-effort by contract: a held lock, a dead Ollama, or
  // SYNAPSE_NO_REFRESH all leave us reading whatever index exists, with freshnessNote() saying so.
  try {
    const paths = freshnessPaths();
    const status = embeddingsStatus({ paths, precise: true });
    FRESHNESS = status.stale
      ? refreshIfStale({ paths, status, log: (m) => process.stderr.write(`${m}\n`) })
      : { refreshed: false, reason: status.reason, status };
  } catch (e) {
    FRESHNESS = null;                                   // freshness must never break the briefing
  }
  ...
  if (note) { lines.push(""); lines.push(note.trimEnd()); }   // say it next to the hits it degrades
  lines.push("");
