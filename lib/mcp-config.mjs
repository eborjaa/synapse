...
if (!["skeleton", "standard", "full", "orchestrator"].includes(surface)) {
  console.error(`mcp-config: --surface must be skeleton|standard|full|orchestrator`);
  process.exit(2);
}
if (!["claude", "cursor", "opencode", "all"].includes(client)) {
  console.error(`mcp-config: --client must be claude|cursor|opencode|all`);
  process.exit(2);
}
...
// --env KEY=VAL (repeatable) plus any extra env already present in a target config. A vault PLUGIN can
// require env of its own (a missing one makes the plugin throw and takes the WHOLE server down, per
// rule-synapse-fail-loudly) — so a var set for one client must not be silently dropped for another.
...
// `mcp` (not `mcpServers`), `command` is an ARRAY (argv), and env is `environment`. Written to
// opencode.json at the project root, merged so an existing model/provider config survives.
const opencodeCfg = {
  $schema: "https://opencode.ai/config.json",
  mcp: { synapse: { type: "local", command: [command], enabled: true, environment: env } },
};
...
// configured is never clobbered; and only when SYNAPSE_OLLAMA_URL / the default points at localhost.
...
if (client === "opencode" || client === "all") {
  ...
  const merged = {
    ...
    mcp: { ...(existingOc.mcp || {}), synapse: opencodeCfg.mcp.synapse },
  };
