#!/usr/bin/env node
// install.mjs — one-step setup for the @eborjaa/synapse context-vault CLI.
//
...
//   - The vault-override env var is SYNAPSE_VAULT; the rc marker is `# @eborjaa/synapse vault agent commands`.
//   - agents.sh SHIPS IN THE PACKAGE now: sourced from its installed location — resolved via
//     import.meta.resolve('@eborjaa/synapse/agents.sh'), else the sibling `../agents.sh` next to this file.
...
function resolveAgentsSh() {
  try {
    const u = import.meta.resolve("@eborjaa/synapse/agents.sh");
...
const SH_MARKER = "# @eborjaa/synapse vault agent commands";
...
const POINTER = `# @eborjaa/synapse context vault
...
briefing instead of reading files ad-hoc. If I name a feature too, add its \`moc-<feature>\`.
...
if (md.includes("# @eborjaa/synapse context vault")) {
...
console.log(`   vault-agents          # every agent command + purpose + default profile`);
console.log(`   vault-mocs            # the feature targets`);
