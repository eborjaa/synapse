...
//
// --write does four idempotent things:
...
if (!write) {
  ...
  try {
    const { targets } = buildMcpTargets({ root: REPO, vaultDir: VAULT });
...
try {
  const { targets, warnings } = buildMcpTargets({ root: REPO, vaultDir: VAULT });
