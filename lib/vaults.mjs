...
/** Write id+root (no tokens) onto the skills volume so DSH can resolve folders without config/. */
export function writeVaultIndex(reg = readRegistry(), dest = join(skillsRoot(), "index.json")) {
  mkdirSync(dirname(dest), { recursive: true });
  const body = {
    version: 1,
    vaults: (reg.vaults || []).map(({ id, root, vaultDir }) => ({ id, root, vaultDir })),
  };
