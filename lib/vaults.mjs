...
//   synapse vaults token <id> [--label L] [--admin]  # mint a bearer credential binding ONE vault
...
function main(argv) {
  ...
  if (sub === "token") {
    ...
    console.log(`\nMinted ${out.scopes.includes("admin") ? "an ADMIN " : "a "}credential for ${out.vaultId}. It is shown ONCE and is not recoverable:\n`);
  ...
  if (sub === "tokens") {
    ...
    for (const t of tokens) {
      ...
      console.log(`  ${(t.label || "(no label)").padEnd(24)} → ${t.vaultId.padEnd(20)} ${t.hash.slice(0, 12)}…${scopes}  ${t.createdAt}`);
    }
